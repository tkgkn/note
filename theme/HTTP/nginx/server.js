...
const app = http.createServer((req, res) => {
  ...
  if (req.url === '/data') {
    res.writeHead(200, {
      // 'Cache-Control': 's-maxage=10' // s-max-age代理的缓存时间，因为通过nginx了，所以会先到代理，查到代理服务器有资源，返回，会比直接请求服务器快。

      'Cache-Control': 'max-age=5 s-maxage=200', // 这里设置了private，只允许浏览器缓存资源，不允许代理缓存，则设置的s-maxage失效。
